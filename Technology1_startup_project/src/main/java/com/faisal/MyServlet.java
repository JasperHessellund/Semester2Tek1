...
public class MyServlet extends HttpServlet {
    ...
    private UserLevel usr = new UserLevel();
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (usr.level(user,pass) ==1) {

            RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/Comment.html");
            dispatcher.forward(request, response);

        }
        if (usr.level(user,pass) == 0) {

            RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/ReadComments.html");
            dispatcher.forward(request, response);
